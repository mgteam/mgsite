...
class FacebookAuthProvider extends AbstractAuthProvider {
	...
	public function normalizeProfile($raw_profile) {
		...
		$response = array_intersect_key(
			$profile,
			array_flip(array( 'id','name','first_name','last_name','username','email','birthday','gender','locale'))
		);
/*array('id','name','first_name','last_name','username','email','birthday','gender','about','address','hometown','relationship_status','friends.fields' => array('id','first_name','last_name','name','email','birthday','gender','address','hometown','relationship_status','username'))*/
		// mapped items
		$map = array(
